...
import type { Series, TimelineData, ConsumptionData, TimelineOptions } from '@src/types/timeline';
...
/**
    ...
    @returns {object} An object with the options for the timeline chart.
*/
export function getTimelineOptions(selectedChart: number): TimelineOptions {
...
/**
  ...
  data.
  @param { Device[]} devices The array with the devices. Needed to identify the chart color.
  @param {ConsumptionData[]} rows The array with the consumption values
  @param {TimelineOptions} timeline Contains options based on the selected chart (minute, hour, day, month, year)
  @return {TimelineData} An object that contains series, labels, colors and the maximun consumption value for the timeline chart.
*/
  ...
  rows.forEach((row, index: number) => {
    ...
    if (cdIndex === -1) {
      ...
      if (dIndex !== -1) {
        ...
        if (devices[dIndex].chartColor) chartColors.push(devices[dIndex].chartColor);
