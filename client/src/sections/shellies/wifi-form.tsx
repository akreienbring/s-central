...
*/
import type { WifiSettings, DeviceTableRow } from '@src/types/device';
...
export default function WifiForm({
  ...
  const [wifiSettings, setwifiSettings] = useState<WifiSettings>(
    type === 'single' ? ({} as WifiSettings) : { ssid: '', password: '' }
  );
  ...
  */
  const handleWifiSettingssReceived = useCallback((msg: SrvAnswerMsg) => {
    if (msg.data.wifisettings) setwifiSettings(msg.data.wifisettings);
  }, []);
  ...
  useEffect(() => {
    if (openWifi)
      if (row && type === 'single' && _.isEmpty(wifiSettings)) {
        ...
        request(requestMsg, handleWifiSettingssReceived);
      }
  ...
  */
  const handleSubmit = (e: React.SubmitEvent) => {
    ...
    if (row) {
      ...
      request(requestMsg, handleWifiUpdateResult);
    }
  };
  ...
  */
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const newWifiSettings: WifiSettings = { ...wifiSettings };
    // TODO: Find a way to do THIS dynamically with typescript!
    //newWifiSettings[e.target.name] = e.target.value;
    if (e.target.name === 'ssid') newWifiSettings[e.target.name] = e.target.value;
    if (e.target.name === 'password') newWifiSettings[e.target.name] = e.target.value;
        ...
        >
          <Stack spacing={3} sx={{ px: 3, py: 3 }}>
            <TextField
              ...
              slotProps={{
                htmlInput: { 'data-testid': 'wifi_ssid_input', maxLength: 25 },
              }}
            />
            <TextField
              required
            ...
            <FormControlLabel
              ...
              control={
                <Checkbox
                  ...
                  onChange={(event) => setIsRiskAccepted(event.target.checked)}
                />
              }
            ...
            <Button
              disabled={!wifiSettings.password || !wifiSettings.ssid || !isRiskAccepted}
              fullWidth
              size="large"
              type="submit"
              color="inherit"
